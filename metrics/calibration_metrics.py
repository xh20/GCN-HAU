...
class ECELoss(nn.Module):
    ...
    def forward(self, logits, labels):
        ...
        if (labels < 0).any():
            index = torch.tensor((labels >= 0).nonzero()[:, 1].clone().detach(), dtype=int)
            softmaxes = F.softmax(logits.index_select(dim=2, index=index), dim=1)
            confidences, predictions = torch.max(softmaxes, 1)
            accuracies = predictions.index_select(dim=1, index=index).eq(labels.index_select(dim=1, index=index))
        else:
