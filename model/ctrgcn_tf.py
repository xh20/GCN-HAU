...
class conv_unit(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=(1, 1), stride=(1, 1), dilation=(1, 1),
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=(pad, 0),
...
class CTRGC(nn.Module):
    ...
    def forward(self, x, A=None, alpha=1):
        ...
        # x1 = self.tanh(torch.matmul(x1.unsqueeze(-1), x2.unsqueeze(-2)))
        # x1 = self.sigmoid(torch.matmul(x1.unsqueeze(-1), x2.unsqueeze(-2)))
...
class TCN_GCN_unit(nn.Module):
    def __init__(self, in_channels, out_channels, A, cat=False, stride=1, residual=True, adaptive=True, kernel_size=5, dilations=[1,2,3,5,7,9]):
...
class LinearBottleneck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, t=2, stride=2, **kwargs):
        ...
        self.block = nn.Sequential(
            ...
            # pw-linear
            nn.Conv2d(in_channels * t, out_channels, kernel_size=(1, 1), bias=False),
            nn.BatchNorm2d(out_channels)
        )
...
class GlobalFeatureExtractor(nn.Module):
    ...
    def __init__(self, in_channels=256, block_channels=(256, 384, 512),
        ...
        if self.residual:
            self.res = nn.Conv2d(in_channels, block_channels[2], (1, 1))
...
class FeatureFusionModule(nn.Module):
    ...
    def __init__(self, num_point, higher_in_channels=256, lower_in_channels=512, out_channels=512, scale_factor=4,
        ...
        self.conv_higher_res = nn.Sequential(
            nn.Conv2d(higher_in_channels, out_channels, kernel_size=(1, 1)),
            nn.BatchNorm2d(out_channels)
        )
...
class Classifer(nn.Module):
    ...
    def __init__(self, in_channels, num_classes, kernel_size=(3, 1), stride=(1, 1), padding=(1, 0), **kwargs):
        ...
        self.conv = nn.Sequential(
            # nn.Dropout(0.1),
            nn.Conv2d(in_channels, num_classes, kernel_size=(1, 1))
        )
        self.feature = None
