...
class Processor():
    ...
    def __init__(self, arg):
        self.arg = arg
        self.model_save_name = os.path.join(arg.model_save_dir, str(arg.model_save_name))
        self.work_dir = os.path.join(arg.work_dir, str(arg.model_save_name))
        self.save_arg()
        if arg.phase == 'train':
            if not arg.train_feeder_args['debug']:
                if os.path.isdir(self.model_save_name):
                    print('log_dir: ', self.model_save_name, 'already exist')
                ...
                self.train_writer = SummaryWriter(os.path.join(self.model_save_name, 'train'), 'train')
                self.val_writer = SummaryWriter(os.path.join(self.model_save_name, 'val'), 'val')
            else:
                self.train_writer = self.val_writer = SummaryWriter(os.path.join(self.model_save_name, 'test'), 'test')
        self.global_step = 0
        self.load_model()
        self.load_optimizer()
        self.load_data()
    ...
    def load_model(self):
        ...
        shutil.copy2(inspect.getfile(Model), self.work_dir)
        print(Model)
        ...
        self.loss = SegmentationLoss(num_class=self.arg.model_args['num_class']).cuda(output_device)
    ...
    def train(self, epoch, save_model=False):
        ...
        if save_model:
            ...
            torch.save(self.model.state_dict(), self.model_save_name + '-' + str(epoch) + '.pt')
        ...
        for ln in loader_name:
            ...
            for batch_idx, (data, label, index) in enumerate(process):
                with torch.no_grad():
                    ...
                    # elif len(output.data.shape) == 3:
                    #     _, predict_label = torch.max(output.data, 2)
                    step += 1
            ...
            if accuracy > self.best_acc:
                ...
                self.best_model = self.model_save_name + '-' + str(epoch) + '-' + str(int(self.global_step)) + '.pt'
                torch.save(self.model.state_dict(), self.model_save_name + '_best' + '.pt')

            # self.lr_scheduler.step(loss)
            print('\nAccuracy: ', accuracy, ' model: ', self.model_save_name)
            if self.arg.phase == 'train':
            ...
            if save_score:
                with open('{}/epoch{}_{}_score.pkl'.format(
                        self.work_dir, epoch + 1, ln), 'wb') as f:
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            self.print_log('\tThe best model is at {} epoch, with accuracy {}, loss {} save as {}.'.format(
                self.best_epoch, self.best_acc, self.best_loss, self.best_model))

        elif self.arg.phase == 'test':
            if not self.arg.test_feeder_args['debug']:
                wf = self.model_save_name + '_wrong.txt'
                rf = self.model_save_name + '_right.txt'
            else:
                wf = rf = None
