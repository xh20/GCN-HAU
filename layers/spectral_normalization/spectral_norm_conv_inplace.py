...
class SpectralNormConv(object):
    ...
    def __init__(self, coeff, name="weight", n_power_iterations=1, eps=1e-12):
        self.coeff = coeff
        self.name = name
        self.reset = False
    ...
    def compute_weight(self, module, do_power_iteration):
        ...
        num_input_dim = self.input_dim[0] * self.input_dim[1] * self.input_dim[2] * self.input_dim[3]
        if u is None or v is None or u.shape != num_input_dim or v.shape != num_input_dim:
            with torch.no_grad():
                num_input_dim = self.input_dim[0] * self.input_dim[1] * self.input_dim[2] * self.input_dim[3]
                v = normalize(torch.randn(num_input_dim, device=weight.device), dim=0, eps=self.eps)

                # forward call to infer the shape
                u = conv2d(v.view(self.input_dim), weight, stride=stride, padding=padding, dilation=dilation, groups=groups,
                           bias=None)
                ...
                u = normalize(torch.randn(num_output_dim, device=weight.device), dim=0, eps=self.eps)
        ...
        if do_power_iteration:
            with torch.no_grad():
                ...
                for _ in range(self.n_power_iterations):
                    ...
                    u_s = conv2d(v.view(self.input_dim), weight, stride=stride, padding=padding, dilation=dilation,
                ...
                if self.n_power_iterations > 0:
                    ...
                    u = u.clone()
                    v = v.clone()
        weight_v = conv2d(v.view(self.input_dim), weight, stride=stride, padding=padding, dilation=dilation,
                          groups=groups, bias=None)
    ...
    def __call__(self, module, inputs):
        ...
        self.input_dim = [1, inputs[0].shape[1], inputs[0].shape[2], inputs[0].shape[3]]
        setattr(
            module, self.name, self.compute_weight(module, do_power_iteration=module.training),
        )

    @staticmethod
    def apply(module, coeff, name, n_power_iterations, eps):
        ...
        fn = SpectralNormConv(coeff, name, n_power_iterations, eps)
        weight = module._parameters[name]
        u = None
        v = None

        # input_dim = [1, weight.shape[0], 120, 26]
        # with torch.no_grad():
        ...
        #     groups = module.groups
        #     # forward call to infer the shape
        #     u = conv2d(v.view(input_dim), weight, stride=stride, padding=padding, dilation=dilation, groups=groups,
        #                bias=None)
    ...
    def __call__(
        ...
        if version is None or version < 1:
            with torch.no_grad():
                ...
                sigma = state_dict[prefix + fn.name + "_sigma"]
                weight_mat = fn.reshape_weight_to_matrix(weight_orig)
                u = state_dict[prefix + fn.name + "_u"]
                v = state_dict[prefix + fn.name + "_v"]
...
def spectral_norm_conv(module, coeff, n_power_iterations, name="weight", eps=1e-12):
    ...
    Example::
        ...
        torch.Size([20])
    """
    SpectralNormConv.apply(module, coeff, name, n_power_iterations, eps)
    return module
